...
import demoPic from '../../images/demoMoviesPicture.svg'
...
function MoviesCard({savedMovies}) {
  ...
  return (
    <article className="card">
      <div className="card__container">
       ...
       { savedMovies ? (
          ...
          <button className={`card__bookmark ${isMarked && 'card__bookmark_marked'}`} onClick={handleBookmarkClick}>
            <svg width="10" height="14" viewBox="0 0 10 14" fill={isMarked ? "#FFFFFF" : "#F4F4F4"} xmlns="http://www.w3.org/2000/svg">
      ...
      <img className="card__img" src={demoPic} alt="Обложка"/>
    </article>
  );
}
